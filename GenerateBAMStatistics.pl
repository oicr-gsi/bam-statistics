...
sub DataSummary {
    ...
    if ( $LineCount % 2 == 0 ) {
        ...
        $ThirdQuartile =
          getMedianLoc( $LineCount - $MedianUpper ) + $MedianUpper;
    ...
    if ( $EvenFirstQuartile == 0 ) {
        $FirstQuartileVal =
          ( $FirstQuartileUpperVal + $FirstQuartileLowerVal ) / 2;
    }

    if ( $EvenThirdQuartile == 0 ) {
        $ThirdQuartileVal =
          ( $ThirdQuartileUpperVal + $ThirdQuartileLowerVal ) / 2;
    }

    close $INPUT_FH;

    open my $STAT_FH, '>', $OutputFileName
      or die "Can't write to '$OutputFileName'\n";
...
# Set up variables
my $InsertHistogramOutput =
  $OutputDir . "/" . $BAMFileName . "-Insert-histogram";
...
my $ReadLengthOutput =
    $OutputDir . "/"
  . $BAMFileName
  . "-read-length.txt";    # Contains the length of all reads
my $NormReadLengthOutput =
  $OutputDir . "/" . $BAMFileName . "-read-length-norm.txt";
my $ReadLengthStatistics =
    $OutputDir . "/"
  . $BAMFileName
  . "-read-length-statistics.txt";    # Contains that statistics of Read Length
my $ReadLengthGraph =
  $OutputDir . "/" . $BAMFileName . "-read-length-graph.png";
my $ReadLengthAvg    = 0;
my $NumOfReads       = 0;
...
open my $READ_LENGTH_STATS_FH, "<", $ReadLengthStatistics
  or die "Couldn't open '$ReadLengthStatistics'\n";
...
my $TempInsertSizeData =
  $OutputDir . "/" . $BAMFileName . "-Insert-size-temp.tsv";
my $InsertSizeStats =
  $OutputDir . "/" . $BAMFileName . "-Insert-size-stats.txt";
my $InsertSizeDataExpanded =
  $OutputDir . "/" . $BAMFileName . "-Insert-size-exp.tsv";
my $InsertSizeAvg = 0;
$NumOfReads = 0;
...
open my $INSERT_VALUES_FH, '<', $InsertSizeData
  or die "Can't read '$InsertSizeData'\n";
open my $INSERT_VALUES_EXP_FH, '>', $InsertSizeDataExpanded
  or die "Can't write to '$InsertSizeDataExpanded'\n";
...
open my $INSERT_VALUES_EXP_R_FH, '<', $InsertSizeDataExpanded
  or die "Could not open '$InsertSizeDataExpanded'\n";
...
open my $INSERT_VALUES_NORM_FH, ">", $TempInsertSizeData
  or die "Can't open '$TempInsertSizeData'\n";
...
open my $INSERT_SIZE_FH, "<", "$InsertSizeStats"
  or die "Cannot open '$InsertSizeStats'\n";
...
# Set up variables
$LineCount = 1;
my $NormalizedCoverageFile =
  $OutputDir . "/" . $BAMFileName . "-normalized-coverage.txt";
my $NormalizedCoverageStats =
  $OutputDir . "/" . $BAMFileName . "-normalized-coverage-stats.txt";
my $NormalizedCoverageAndGCFile =
  $OutputDir . "/" . $BAMFileName . "-normalized-coverage-gc.txt";
...
# Parse output from R and store to output CSV
open my $COVERAGE_BIAS_FH, "<", "$NormalizedCoverageStats"
  or die "Cannot open '$NormalizedCoverageStats'\n";
...
open my $COVERAGE_AND_GC_FH, "<", $NormalizedCoverageAndGCFile
  or die "Can't open '$NormalizedCoverageAndGCFile'\n";
...
$LineCount = 1;
my $BaseQualityDist =
  $OutputDir . "/" . $BAMFileName . "-Base-quality-dist.txt";
my $TempBaseQualityDist =
  $OutputDir . "/" . $BAMFileName . "-Base-quality-dist-temp.txt";
my $BaseQualityDistStats =
  $OutputDir . "/" . $BAMFileName . "-Base-quality-dist-stats.txt";
...
open my $BASE_QUALITY_STATS_FH, "<", "$BaseQualityDistStats"
  or die "Cannot open '$BaseQualityDistStats'\n";
